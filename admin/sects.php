...
$page = (int) (!isset($_GET["page"]) ? 1 : $_GET["page"]);
	...
	$qe='ASC';
	if($key)
	$searchkey= "where sect LIKE '%$key%'";
	else 
	$searchkey='where 1';

$sth=$conn->prepare("SELECT * from sects $searchkey order by sect {$qe} ");
try{$sth->execute();}
catch(Exception $e){
echo $e->getMessage();
}
...
$sth=$conn->prepare("SELECT * from sects $searchkey order by sect {$qe} LIMIT {$startpoint}, {$limit}");
try{$sth->execute();}
catch(Exception $e){
echo $e->getMessage();
}
...
<?php require_once("sidebar.php"); ?>
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
		...
		<div class="col-md-3">
		<h3> Sects
		<a href="#" class="newcommunity" data-no-turbolink ='true' data-toggle="modal" data-target="#sects" data-remote="true">
...
<div class="col-md-3" style="text-align:right;">			
		...
		<ul class="pagination">
			...
			<div class="content-panel pn">
				<div id="profile-01">
					<h3 style='text-transform:uppercase;'><?php echo $row['sect']; ?></h3><br>
				</div>
				
			<div class="pr2-social centered">
				<a href="#" class="editsect" data-no-turbolink ='true' cid="<?php echo $row['id']; ?>" data-toggle="modal" data-target="#edit-sec" data-remote="true">
				<i class="fa fa-pencil"></i></a>
				<!-- <a href="#" class="delcommunity" data-no-turbolink ='true' data-toggle="m data-remote="true"odal" data-target="#communities" data-remote="true">
...
<?php require_once('add_sects.php'); ?>
...
<script>
	$('.editsect').on('click',function(){
		...
		function(data){
	       ...
	       var field='<form action="eventHandler.php" method="post"><p>Enter Sect Name</p><input type="text" name="sect" value="'+v.sect+'" class="form-control" required>  <input type="hidden" name="event" value="edit_sect"><input type="hidden" name="cid" value="'+cid+'"><div class="modal-footer"><button data-dismiss="modal" class="btn btn-default" type="button">Cancel</button><button class="btn btn-theme" type="submit">Submit</button></div></form>';

	       $('#sect-editing1').append(field).fadeIn(1000);
	   });
...
<div class="modal fade" id="edit-sec" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<h4 class="modal-title" id="myModalLabel">Edit Sect</h4>
			</div>
			<div class="modal-body">
				<div class="" id="sect-editing1">
