...
$page = (int) (!isset($_GET["page"]) ? 1 : $_GET["page"]);
	...
	$qe='ASC';
	if($key)
	$searchkey= "where education LIKE '%$key%'";
	else 
	$searchkey='where 1';

$sth=$conn->prepare("select * from educations $searchkey order by education {$qe}");
try{$sth->execute();}
catch(Exception $e){
echo $e->getMessage();
}
...
$sth=$conn->prepare("select * from educations $searchkey order by education {$qe} LIMIT {$startpoint}, {$limit}");
try{$sth->execute();}
catch(Exception $e){
echo $e->getMessage();
}
...
<?php require_once("sidebar.php"); ?>
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
		...
		<div class="col-md-3"> 
		<h3> Education Levels
		<a href="#" class="education" data-no-turbolink ='true' data-toggle="modal" data-target="#education">
...
<div class="col-md-3" style="text-align:right;">			
		...
		<ul class="pagination" >
			...
			<div class="content-panel pn">
				<div id="profile-01">
					<h3 style='text-transform:uppercase;'><?php echo $row['education']; ?></h3><br>
				</div>
				
			<div class="pr2-social centered">
				
				<a href="#" class="editedu" data-no-turbolink ='true' cid="<?php echo $row['id']; ?>" data-toggle="modal" data-target="#editeducation">
 ...
 <script>
	$('.editedu').on('click',function(){
		...
		function(data){
	       ...
	       var field='<form action="eventHandler.php" method="post"><p>Enter Education Level</p><input type="text" name="education" value="'+v.education+'" class="form-control" required>  <input type="hidden" name="event" value="edit_education"><input type="hidden" name="cid" value="'+cid+'"><div class="modal-footer"><button data-dismiss="modal" class="btn btn-default" type="button">Cancel</button><button class="btn btn-theme" type="submit">Submit</button></div></form>';

	       $('#edu-editing1').append(field).fadeIn(1000);
	   });
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel">Edit Education Level</h4>
			</div>
			<div class="modal-body">
				<div class="" id="edu-editing1">
...
<?php require_once('add_education.php'); ?>