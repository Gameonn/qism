...
$page = (int) (!isset($_GET["page"]) ? 1 : $_GET["page"]);
	...
	$qe='ASC';
	if($key)
	$searchkey= "where ancestral_origin LIKE '%$key%'";
	else 
	$searchkey='where 1';

$sth=$conn->prepare("SELECT * FROM `ancestral_origins` $searchkey order by ancestral_origin {$qe}");
try{$sth->execute();}
catch(Exception $e){
echo $e->getMessage();
}
...
$sth=$conn->prepare("SELECT * FROM `ancestral_origins` $searchkey order by ancestral_origin {$qe} LIMIT {$startpoint}, {$limit}");
try{$sth->execute();}
catch(Exception $e){
echo $e->getMessage();
}
...
<?php require_once("sidebar.php"); ?>
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
		...
		<div class="col-md-3">
		<h3>Ethnic Origins
		<a href="#" class="origins" data-no-turbolink ='true' data-toggle="modal" data-target="#origin" data-remote="true">
...
<div class="col-md-3" style="text-align:right;">			
		...
		<ul class="pagination">
			...
			<div class="content-panel pn">
				<div id="profile-01">
					<h3 style='text-transform:uppercase;'><?php echo $row['ancestral_origin']; ?></h3><br>
				</div>
				
			<div class="pr2-social centered">
				<a href="#" class="edit_origin" data-no-turbolink ='true' cid="<?php echo $row['id']; ?>" data-toggle="modal" data-target="#editorigin">
      ...
      <footer class="site-footer">
          <div class="text-center">
              QISMA
              <a href="ethnic_origins.php#" class="go-top">
 ...
 <script>
	$('.edit_origin').on('click',function(){
		
		var cid= $(this).attr('cid');
		var event='get-origin';
		...
		function(data){
			...
			console.log(data);
			$('#origin-editing1').empty();
			$.each(data, function(index,v) {
			
	       var field='<form action="eventHandler.php" method="post"><p>Enter Ethnic Origin Name</p><input type="text" name="origin" value="'+v.ancestral_origin+'" class="form-control" required>  <input type="hidden" name="event" value="edit_origin"><input type="hidden" name="cid" value="'+cid+'"><div class="modal-footer"><button data-dismiss="modal" class="btn btn-default" type="button">Cancel</button><button class="btn btn-theme" type="submit">Submit</button></div></form>';

	       $('#origin-editing1').append(field).fadeIn(1000);
	   });
 ...
 <div class="modal fade" id="editorigin" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<h4 class="modal-title" id="myModalLabel">Edit Ethnic Origin</h4>
			</div>
			<div class="modal-body">
				<div class="" id="origin-editing1">
...
<?php require_once('add_ethnic_origins.php'); ?>